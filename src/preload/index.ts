...
const dbApi: DBApi = {
  ...
  // Profile management methods
  updateProfile: (data: { id: number; name: string; email: string; phone: string | null }) => 
    ipcRenderer.invoke('db:staff-updateProfile', data),

  changePassword: (data: { id: number; oldPassword: string; newPassword: string }) => 
    ipcRenderer.invoke('db:staff-changePassword', data),
